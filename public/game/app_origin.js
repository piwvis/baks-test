...
var background, bird, ground, pipe, prize, coin, poop, coins, poops, bottomPipe, boxes, pipes, rotationDelta, counter,
  counterOutline, highScore, highScoreOutline;
var prizeBox;
var started = false;
...
function handleComplete() {
  ...
  if (window.eventService.getUser()) {
    highScore.text = window.eventService.getUser().bestScore;
    highScoreOutline.text = window.eventService.getUser().bestScore;
  }
...
function tick(event) {
  ...
  if (bird.y > (ground.y - 40)) {
    if (!dead) {
      console.log(window.eventService.getHealthSize());
      if (window.eventService.getHealthSize() > 0) {
